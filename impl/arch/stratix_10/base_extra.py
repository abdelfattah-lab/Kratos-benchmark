...
def gen_arithmetic_2chains():
    return """<mode name="arithmetic_2chains">
                    <pb_type name="arithmetic" num_pb="1">
                      <input name="in" num_pins="5"/>
                      <input name="cin" num_pins="2"/>
                      <output name="out" num_pins="2"/>
...
DEFAULTS = {
    'enable_arithmetic_2chains': False,
}

class LUTBaseExtraArchFactory(ArchFactory, ParamsChecker):
    def get_name(self, enable_arithmetic_2chains: bool, **kwargs):
        name = f"type.s10-extra_carry-"
        if not enable_arithmetic_2chains:
            name += "2chains_off"
        return name
    ...
    def get_arch(self,
                 enable_arithmetic_2chains: bool,
                 **kwargs):
        return TEMPLATE.format(
            arithmetic_2chains=gen_arithmetic_2chains() if enable_arithmetic_2chains else ''
        )
    
    # TODO: figure out what is meant by netstats, if need to be updateed
