...
class BaseSweepArchFactoryOld(ArchFactory, ParamsChecker):
    """
    Legacy version of the sweep architecture that evenly partitions LAB feedback.
    """
    ...
    def verify_params(self, params):
        ...
        if fle_per_clb <= 0:
            raise ValueError("fle_per_clb must be a positive integer.")
        if fle_per_clb > 10:
            raise ValueError("fle_per_clb > 10 is not supported by BaseSweepArchFactoryOld.")
        filled['fle_per_clb'] = fle_per_clb
        return filled
