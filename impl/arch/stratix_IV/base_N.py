  ...
  <complexblocklist>
    ...
    <pb_type name="clb">
      ...
      <!-- Describe fracturable logic element.  
        ...
        -->
      <pb_type name="fle" num_pb="{num_pb_fle}">
...
{delay_constant_xbH}
        ...
        <!-- This way of specifying direct connection to clb outputs is important because this architecture uses automatic spreading of opins.  
                 ...
                 naive specification).
          -->
        <direct name="clbouts1" input="fle[{fle_last_index}:0].out[0:0]" output="clb.O[{clb_out_range_1}]"/>
        ...
        <direct name="carry_out" input="fle[{fle_last_index}:{fle_last_index}].cout" output="clb.cout">
          <pack_pattern name="chain" in_port="fle[{fle_last_index}:{fle_last_index}].cout" out_port="clb.cout"/>
        </direct>
        <direct name="carry_link" input="fle[{fle_2last_index}:0].cout" output="fle[{fle_last_index}:1].cin">
          <pack_pattern name="chain" in_port="fle[{fle_2last_index}:0].cout" out_port="fle[{fle_last_index}:1].cin"/>
        </direct>
      </interconnect>
    </pb_type>
...
"""
def generate_arch(CLB_pins_per_group: int, num_feedback_ble: int, lut_size: int, ble_count: int) -> str:
    config_dict = {}
    lut_size_small = lut_size - 1
    lut_size_large = lut_size
    ...
    config_dict['clb_out_range_1'] = f"{ble_count - 1}:0"
    config_dict['clb_out_range_2'] = f"{num_opins_clb - 1}:{ble_count}"
    config_dict['fle_last_index'] = ble_count - 1
    config_dict['fle_2last_index'] = ble_count - 2
    #test
    ...
    # feedback
    index_ = 'ABCDEFGH'
    feedback_group_pin_index = distribute_pins(ble_count, int(ble_count/2), 8)
    for i in range(8):
        j = i%len(feedback_group_pin_index)
        config_dict[f'feedback_xb{index_[i]}'] = ' '.join([f'fle[{x}:{x}].out' for x in feedback_group_pin_index[j]])
        config_dict[f'delay_constant_xb{index_[i]}'] = '\n'.join([f'<delay_constant max="75e-12" in_port="fle[{x}:{x}].out" out_port="fle.in[{i}:{i}]"/>' for x in feedback_group_pin_index[j]])
...
class BaseNArchFactory(ArchFactory, ParamsChecker):
    """
    ...
    def get_name(self, CLB_pins_per_group: int, num_feedback_ble: int, lut_size: int, ble_count: int, **kwargs) -> str:
      return f"clb.{CLB_pins_per_group}_ble.{num_feedback_ble}_lut.{lut_size}_N.{ble_count}"
